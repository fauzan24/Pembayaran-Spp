...
<table class="table table-striped table-bordered">
    ...
    foreach($query as $data){ 
        ...
        $bulan_dibayar = mysqli_query($koneksi, "SELECT bulan_dibayar FROM pembayaran WHERE nisn = '$data[nisn]' ORDER BY bulan_dibayar ASC");
        ?>
        <tr>
            <td><?= $no++; ?></td>
            <td><?= $data['nisn'] ?></td>
            ...
            <td>
            <?php
                if(mysqli_num_rows($bulan_dibayar) > 0){
                    while($row_bulan = mysqli_fetch_assoc($bulan_dibayar)){
                        echo $row_bulan['bulan_dibayar'] . "<br>";
                    }
                } else {
            ...
            <td><b><?= number_format($data['nominal'],2,',','.');?></b></td>
            ...
            <td>
                ...
                if($kekurangan == 0){
                    ...
                    <a href="?url=tambah-pembayaran&nisn=<?= $data['nisn'] ?>&kekurangan=<?= $kekurangan ?>&tahun=<?= $data['tahun'] ?>" class="btn btn-danger">Pilih dan Bayar</a>
                <?php } ?>
            </td>
            <td>
